...
function buildStepNodes(step: ActionTask): FlowNode[] {
  return getStepMessages(step).map((message, index, messages) => ({
    id: message.id,
...
export default function PlanFlow({ steps, offers }: PlanFlowProps) {
  ...
  const [currentStepIndex, setCurrentStepIndex] = useState(() =>
    Math.min(initialProgress.startIndex, Math.max(steps.length - 1, 0)),
  );
  const [completedCount, setCompletedCount] = useState(initialProgress.completed);
